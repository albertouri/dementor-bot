...
map<int, int> computeAssignments(RectangleArray< double> &cost)
{
  ...
  // i < n is hack; algorithm sometimes entered infinite loop at the game end
  // possibly because of workers hack to gather properly after some situations (ie base defend)
  for(int i = 0; i < 50; i++)
  {
	  if (i == 49)
	  {
		  BWAPI::Broodwar->printf("ERR: HungAlg reached maximum!");
	  }
