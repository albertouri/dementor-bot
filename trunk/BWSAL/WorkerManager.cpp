...
void WorkerManager::updateWorkerAssignments()
{
  ...
  assignment=computeAssignments(cost);
	...
	{
		// hack, crashovalo to ked pristupovalo na vector mimo rozsah... lololo
		if (workerUnit.size() <= a->first)
		{
			continue;
		}

		if (taskUnit.size() <= a->second)
		{
			continue;
		}
...
void WorkerManager::rebalanceWorkers()
{
  ...
  for(set<Base*>::iterator b = this->basesCache.begin(); b != this->basesCache.end(); b++)
  {
	...
	sort(baseMineralOrder.begin(), baseMineralOrder.end(), utils::mineralCompare);
  ...
  if (!mineralOrder.empty())
  {
    //order minerals by score (based on distance and resource amount)
	  sort(mineralOrder.begin(), mineralOrder.end(), utils::mineralCompare);
  ...
  {
		UnitGroup workers = SelectAll()
		(isWorker)
		(isIdle);

		UnitGroup minerals = AllUnits()
		(Mineral_Field);

		UnitGroup::iterator workersIt, mineralsIt;
		mineralsIt = minerals.begin();

		if (mineralsIt != minerals.end())
		{
			BWAPI::Unit *minerals = (*mineralsIt);

			for (workersIt = workers.begin(); workersIt != workers.end(); workersIt++)
			{
				BWAPI::Unit *worker = (*workersIt);

				worker->rightClick(minerals);
			}
		}
...
bool WorkerManager::isAutoBuildEnabled()
{
	return autoBuild;
}
